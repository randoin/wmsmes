...
$tgl=date("Y-m-d");
	...
	$tampil=mysql_query("select * from hargasewa where asperindo='0' AND type='import' order by id DESC limit 1");
	$p=mysql_fetch_array($tampil);
?>	

<h2>HARGA SEWA DAN SHARING IMPORT NON ASOSIASI</h2>  
<form method="POST" action='?module=sewagudangimnonasosiasi'>
<table>
	<tr>
    	<td align="center" colspan="2"><b>IMPORT NON ASOSIASI</b></td>
    </tr>
...
<table>
	<tr>
	 ...
	 //error_reporting(E_ALL);
		...
		$tampil=mysql_query("SELECT * FROM hargasewa where asperindo='0' AND type='import' ORDER BY id DESC limit $posisi,$batas");
    	$no     = $posisi+1;
		//$tgl1=my2date($r['tgl']);
   ...
   echo "</table>";
			...
			$jmldata      = mysql_num_rows(mysql_query("SELECT * FROM hargasewa where asperindo='0' AND type='import'"));
